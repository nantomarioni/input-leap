...
// Minimal local logging macro to avoid depending on the project's logging
// implementation which pulls heavy platform-specific headers when included
// from this simple fork implementation.
#ifndef LOG_DEBUG
#define LOG_DEBUG(...) do { std::fprintf(stderr, __VA_ARGS__); std::fprintf(stderr, "\n"); } while(0)
#endif
