...
public:
    ...
    // Return a pointer to a loaded extension instance, or nullptr if not present
    ScreenExtensionPtr getExtension(const std::string& name) const;
    // Dispatch a command to extensions. Iterate in registration order and stop at first that handles it.
    bool dispatchCommandToExtensions(const std::string& cmd, const OptionsList& args);
