    ...
    if (hdc != nullptr) {
        if (dim && !m_isDimmed) {
            ...
            if (GetDeviceGammaRamp(hdc, m_originalGamma)) {
                ...
                LOG_DEBUG("successfully got gamma ramp, creating %d%% dimmed version", m_dimmingPercentage);
                ...
                if (SetDeviceGammaRamp(hdc, dimmedGamma)) {
                    m_isDimmed = true;
                    LOG_DEBUG("screen dimmed to %d%% brightness successfully", m_dimmingPercentage);
                } else {
